...
class _StorePageState extends State<StorePage> {
  ...
  final Map<int, List<_StoreItemData>> _storeLevelsData = const {
    1: [
      _StoreItemData(
        icon: Icons.grain,
        title: 'Garam Dapur 250g',
        description:
            'Garam beryodium konsumsi keluarga untuk bumbu dapur sehari-hari.',
        price: 2500,
        locked: false,
      ),
      _StoreItemData(
        icon: Icons.clean_hands,
        title: 'Sabun Cuci Tangan 100ml',
        description: 'Sabun cuci tangan cair wangi pembasmi kuman dan bakteri.',
        price: 3500,
        locked: false,
      ),
    ],
    2: [
      _StoreItemData(
        ...
        title: 'Spons Cuci Piring 2in1',
        description: 'Spons cuci piring busa tebal dan sabut penggosok kerak.',
        price: 5000,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.local_fire_department,
        title: 'Korek Api Kayu (1 Pack)',
        description: 'Korek api kayu praktis untuk kebutuhan dapur.',
        price: 4000,
        locked: true,
      ),
    ],
    3: [
      _StoreItemData(
        ...
        title: 'Kain Lap Microfiber',
        description:
            'Kain lap microfiber menyerap air tinggi untuk membersihkan meja & piring.',
        price: 7500,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.soap,
        title: 'Sabun Cuci Piring 210ml',
        description:
            'Cairan pencuci piring ekstrak jeruk nipis ampuh hilangkan lemak.',
        price: 9000,
        locked: true,
      ),
    ],
    4: [
      _StoreItemData(
        ...
        title: 'Sikat Botol Gagang Panjang',
        description: 'Sikat fleksibel pembersih dalam botol minum dan gelas.',
        price: 12000,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.takeout_dining,
        title: 'Gula Pasir 250g',
        description: 'Gula pasir putih murni kemasan praktis 250 gram.',
        price: 15000,
        locked: true,
      ),
    ],
    5: [
      _StoreItemData(
        ...
        title: 'Kantong Sampah Ramah Lingkungan',
        description: '1 roll plastik sampah biodegradable mudah terurai.',
        price: 20000,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.wash,
        title: 'Deterjen Bubuk 450g',
        description: 'Deterjen pembersih pakaian harum anti bau apek.',
        price: 25000,
        locked: true,
      ),
    ],
    6: [
      _StoreItemData(
        icon: Icons.opacity,
        title: 'Minyak Goreng Pouch 500ml',
        description: 'Minyak goreng kelapa sawit jernih kemasan pouch 500ml.',
        price: 35000,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.local_drink,
        title: 'Botol Minum Plastik BPA Free',
        description:
            'Botol minum portable ramah lingkungan bebas bahan kimia BPA.',
        price: 40000,
        locked: true,
      ),
    ],
    7: [
      _StoreItemData(
        ...
        title: 'Beras Premium 1 Kg',
        description: 'Beras putih pulen berkualitas super kemasan 1 kg.',
        price: 50000,
        locked: true,
      ),
      _StoreItemData(
        icon: Icons.egg,
        title: 'Telur Ayam Segar 1/2 Kg',
        description:
            'Telur ayam negeri fresh kaya protein untuk konsumsi keluarga.',
        price: 45000,
        locked: true,
      ),
    ],
  ...
  Future<void> _saveRedeemedItem(String title) async {
    ...
    try {
      ...
      await prefs.setStringList(
        'ecopoint_redeemed_store_items',
        _redeemedTitles.toList(),
      );
    } catch (_) {}
  }
  ...
  void _onTukarPoint(_StoreItemData item) {
    if (item.locked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Item ini masih terkunci! Tingkatkan EcoTree Level kamu terlebih dahulu.',
    ...
    if (WalletState.instance.currentPoints < item.price) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Poin tidak mencukupi untuk menukar "${item.title}"! Poin kamu: ${WalletState.instance.currentPoints} Pts, Butuh: ${item.formattedPrice}',
            style: _jakarta(color: Colors.white),
          ),
          backgroundColor: Colors.red,
          behavior: SnackBarBehavior.floating,
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Tukar ${item.formattedPrice} untuk mendapatkan "${item.title}"?',
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFF4C8C2B),
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              final success = await context.read<UserProvider>().redeemPoints(
                item.price,
              );
              if (success) {
                ...
                NotificationState.instance.addNotification(
                  ...
                  subtitle:
                      'Penukaran "${item.title}" (${item.formattedPrice}) berhasil diajukan. Status: Menunggu Admin. Kamu akan dikabari via Chat ketika diterima.',
                );
                HistoryState.instance.addHistory(
                  title: 'Tukar Barang: ${item.title}',
                  description:
                      'Menunggu konfirmasi admin (Akan dikabari via Chat)',
                  category: 'EcoStore',
                  ...
                  showDialog(
                    ...
                    builder: (c) => AlertDialog(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text(
                            'Permintaan penukaran poin untuk "${item.title}" (${item.formattedPrice}) berhasil diajukan.',
                            style: _jakarta(fontSize: 13.5),
                          ),
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFFE0A800), Color(0xFFF0C230)],
        ),
  ...
  Widget _buildStoreLevelSection(
    int levelNum,
    List<_StoreItemData> items,
    int userTreeLevel,
  ) {
    final bool isLevelUnlocked = levelNum == 1
        ? true
        : userTreeLevel >= levelNum;
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  data.title,
                  style: _jakarta(fontSize: 13.5, fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 4),
                Text(
                  data.description,
          ...
          Column(
            ...
            children: [
              Text(
                data.formattedPrice,
                style: _jakarta(fontSize: 12, fontWeight: FontWeight.w600),
              ),
